-- Count known users case-insensitively. Sleeper usernames are
-- case-insensitive, so "Skjjcruz" and "skjjcruz" are one person; the
-- Mission Control tile was double-counting them (owner report 2026-07-30).
-- Re-issues admin_analytics_report with lower(username) in the distinct
-- counts. Safe to re-run (create or replace).
...
funnel_counts as (
  select
    ...
    count(distinct lower(s.username)) filter (where s.username is not null) as users
...
select jsonb_build_object(
  ...
  'totals', (
    select jsonb_build_object(
      ...
      'knownUsers', count(distinct lower(username)) filter (where username is not null),
