-- Count people by account, not just Sleeper username (owner ask 2026-08-03).
-- Members signed in by email/Google carry a user_id on every analytics event
-- but may have no Sleeper username attached on that device, so the previous
-- username-only distinct filed them as anonymous ("0 known users" while the
-- backend logged real sign-ins). This re-issues admin_analytics_report with a
-- person_key: the account id when present, else the account an identical
-- username maps to elsewhere in the window, else the lowercased username —
-- so one human on two devices still counts once. Safe to re-run.
...
identity_links as (
  -- Window-local username -> account bridge: any event carrying both sides
  -- links that username to the account for the whole report.
