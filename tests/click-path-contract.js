...
const surfaces = [
  ...
  },
  {
    ...
    checks: [
      ...
      ['onClick={e => { e.stopPropagation(); openDraftPlayer(pid); }}', 'draft preview pick rows open player detail (live-aware pid)'],
      ["onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openDraftPlayer(line.pid); } }}", 'draft rows support keyboard activation'],
